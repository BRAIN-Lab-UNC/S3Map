...
if __name__ == "__main__":    
    ...
    # n_vertexs = [10242, 40962]

    # start distortion correction for each level
    for n_vertex in n_vertexs:
        n_level = n_vertexs.index(n_vertex)
        print('\n------------------------------------------------------------------')
        print("Starting distortion correction on {}-th level with {} vertices.".format(n_level+1, n_vertex))
        ...
        model.to(device)
        model_file = os.path.join(model_path, 'S3Map_'+hemi+'_ver'+ str(n_vertex) + '_area'+ str(weight_area) + \
        ...
        for batch_idx, (inner_vert, file) in enumerate(test_dataloader):
            ...
            if save_interim_results:
                ...
                loss_dist = (moved_dist - inner_dist) / (inner_dist+1e-12)
                loss_area = (moved_area - inner_area) / (inner_area+1e-12)
                moved_surf['dist_dis'] = loss_dist.cpu().numpy()
                moved_surf['area_dis'] = loss_area.cpu().numpy()
            ...
            orig_sphere_moved = moveOrigSphe(template, orig_sphe_surf, moved_surf, orig_inner_surf)
